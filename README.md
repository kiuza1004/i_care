...
| 항목 | 데이터 |
|---|---|
| 최초 PWA 설치 (모든 정적 리소스) | ~10KB |
| 이후 앱 실행 (캐시 사용) | 0KB |
| 1회 위치 요청 (요청 + 응답 + 좌표) | ~1–2KB |
| 한 달 30회 사용 가정 | ~60KB |
...
```
/request   { active: bool, timestamp: ms }   # 부모가 씀, 아이가 읽음
/response  { status: "accepted"|"denied", timestamp: ms }
/location  { lat, lng, accuracy, timestamp: ms }
```
